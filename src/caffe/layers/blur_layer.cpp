...
#include "caffe/permutohedral.hpp"
...
void BlurLayer<Dtype>::LayerSetUp(const vector<Blob<Dtype>*>& bottom,
  ...
  BlurParameter blur_param = this->layer_param_.blurfilter_param();
  CHECK(!blur_param.has_kernel_size() !=
      !(blur_param.has_kernel_h() && blur_param.has_kernel_w()))
      << "Filter size is kernel_size OR kernel_h and kernel_w; not both";
  CHECK(blur_param.has_kernel_size() ||
      (blur_param.has_kernel_h() && blur_param.has_kernel_w()))
      << "For non-square filters both kernel_h and kernel_w are required.";
  CHECK((!blur_param.has_pad() && blur_param.has_pad_h()
      && blur_param.has_pad_w())
      || (!blur_param.has_pad_h() && !blur_param.has_pad_w()))
      << "pad is pad OR pad_h and pad_w are required.";
  CHECK((!blur_param.has_stride() && blur_param.has_stride_h()
      && blur_param.has_stride_w())
      || (!blur_param.has_stride_h() && !blur_param.has_stride_w()))
      << "Stride is stride OR stride_h and stride_w are required.";
  if (blur_param.has_kernel_size()) {
    kernel_h_ = kernel_w_ = blur_param.kernel_size();
  } else {
    kernel_h_ = blur_param.kernel_h();
    kernel_w_ = blur_param.kernel_w();
  }
  CHECK_GT(kernel_h_, 0) << "Filter dimensions cannot be zero.";
  CHECK_GT(kernel_w_, 0) << "Filter dimensions cannot be zero.";
  if (!blur_param.has_pad_h()) {
    pad_h_ = pad_w_ = blur_param.pad();
  } else {
    pad_h_ = blur_param.pad_h();
    pad_w_ = blur_param.pad_w();
  }
  if (!blur_param.has_stride_h()) {
    stride_h_ = stride_w_ = blur_param.stride();
  } else {
    stride_h_ = blur_param.stride_h();
    stride_w_ = blur_param.stride_w();
  }
  // Configure output channels and groups.
  channels_ = bottom[0]->channels();
  num_output_ = this->layer_param_.blurfilter_param().num_output();
  CHECK_GT(num_output_, 0);
  group_ = this->layer_param_.blurfilter_param().group();
  ...
  bias_term_ = this->layer_param_.blurfilter_param().bias_term();
  if (this->blobs_.size() > 0) {
    ...
    shared_ptr<Filler<Dtype> > weight_filler(GetFiller<Dtype>(
        this->layer_param_.blurfilter_param().weight_filler()));
    ...
    if (bias_term_) {
      ...
      shared_ptr<Filler<Dtype> > bias_filler(GetFiller<Dtype>(
          this->layer_param_.blurfilter_param().bias_filler()));
