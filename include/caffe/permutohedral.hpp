...
namespace caffe {
  ...
  class Permutohedral
  {
    ...
    private:
      ...
      typedef typename boost::array<float, 7> filter_type;
      //const filter_type filter_;
      filter_type filter_;

      typedef boost::array<int, (filter_type::static_size
          - 1) * 2> Neighbors;
    ...
    public:
      Permutohedral() {
        ...
        //filter_ = {{924, 729, 495, 220, 66, 12, 1}};

        filter_type filter = {{924, 729, 495, 220, 66, 12, 1}};
        filter_ = filter;
      }
      ...
      void init(const value_type* feature, int data_count, int
        ...
        float inv_std_dev = sqrt(1.0 / 6.0
            + (filter_type::static_size - 1) / 2.0)*(d_+1);
      ...
      void blur(std::vector<value_type>& splatted, const std::size_t value_size,
          std::vector<value_type>& blurred) const {
        // allow move
        std::swap(splatted, blurred);
